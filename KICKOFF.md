# Kickoff meeting
This document is written in Markdown.  

## Proposed date: Saturday 12th September 2020
## Proposed time: 13:00 UTC (14:00 UK @ UTC + 1, 15:00 Germany/Netherlands @ UTC + 2)
## Video conference software: Jitsi @ https://jitsi.org/ (no download required)

## Invitees
No commitment in terms of time or anything else is expected!  
This is purely a kickoff meeting for potential stakeholders.  
  [✓] Jake +49-176-34548857: Software developer, Munich/Amsterdam  
  [✓] Malka +49-163-2716301: Forensics student, Amsterdam  
  [✓] David +44-7837-616048: Lawyer, London  
  [✓] Ruben +49-176-67805663: Informatik student & activist, Munich  
  [✓] Nahom +44-7751-655971: London  
  [✓] Johnny +44 7531 368067: XR activist, London  
  [ ] Johnny: BLM activist, Munich  
  [ ] Ben: Software developer, London  
  [ ] Emma: London  
  [ ] Will: London  
  [ ] Becca: Newcastle  
  [X] Jannis: Munich  

## Project overview
An open-source anti-racist research project on the police state, with the potential for future monetization.
### Goals
#### Mobile phone app (PWA)
Provide a user at-zero-cost with essential services for interaction with law enforcement officials:
* Audio/Location upload streaming
* Legal guide:        traffic stop, border control, stop-and-search, ...
* 'Kontrolle' upload: when, where, by whom, for what reason, ...

#### Desktop/tablet website
Allow a user at-zero-cost to access data which they have uploaded.
* From user:         ethnicity/gender (optional), incidental audio/photo/video recording
* From user's phone: nickname, date/time, location, 
* From the police:   id, name/rank/station, statements, audio/photo/video recording (optional)

#### Public-access database
Allow access to researchers and activist groups, either at-zero-cost or affordably-priced.  
Data may also enable development of active countermeasures to overpolicing.  
### Organisation
#### Roles
* Software development
* Product ownership
* System architechture
* Dev-ops
* Legal
* Organisers:  England, Bavaria, Holland
* Translators: English, Polish, German, Dutch
* Marketing
* ...

### Current state
Dummy mobile phone app live @ https://www.mostlyharmful.org/home.  
First release candidate is under construction.  
Readme document for open-source has been drafted.

## Agenda
### Agree on decision-making process
### Decide who does which role(s)
### Organisational planning
### Technical planning
### ...
### Set up next meeting, with action items